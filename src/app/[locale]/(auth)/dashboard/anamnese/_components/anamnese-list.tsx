...
import AddAnamneseButton from "./add-anamnese-button";
...
export function AnamneseList() {
  ...
  }, [dateRange, router, searchParams, queryClient]);

  const { data: anamneses, isLoading } = useQuery({
    queryKey: ["anamneses", search, sorting[0].id, (sorting[0].desc ? "desc" : "asc"), dateRange?.from?.toISOString(), dateRange?.to?.toISOString()],
  ...
  if (isLoading || status === 'executing') return <Skeleton className="h-[300px] w-full" />;
  ...
  return (
    <>
      <div className="mb-4 flex items-center justify-between gap-4">
        ...
        <AddAnamneseButton onAnamneseUpsertSuccess={handleSuccess} />
      </div>
      <DataTable
        columns={columns}
        data={anamneses?.data || []}
