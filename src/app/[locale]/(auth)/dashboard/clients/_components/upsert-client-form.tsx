    ...
    useAction(getCnpjInfo, {
      onSuccess: (response) => {
        if (response.success) {
          form.setValue(
            "razaoSocial",
            (response.data.nome || "").toUpperCase(),
          );
          form.setValue(
            "fantasia",
            (
          ...
          form.setValue(
            "numero",
            (response.data.numero || "").toUpperCase(),
          );
          form.setValue(
            "complemento",
            (response.data.complemento || "").toUpperCase(),
          );
          form.setValue(
            "bairro",
          ...
          form.setValue(
            "cidade",
            (response.data.municipio || "").toUpperCase(),
          );
          form.setValue("uf", (response.data.uf || "").toUpperCase());
          form.setValue("cep", response.data.cep || "");
          form.setValue("ie", (response.data.ie || "").toUpperCase());
          form.setValue(
            "cnae",
            (response.data.cnae_fiscal || "").toUpperCase(),
          );
          form.setValue("telefone1", response.data.telefone || "");
          form.setValue("email", response.data.email || "");

          const rawCpfCnpj = response.data.cnpj || response.data.cpf;
          if (rawCpfCnpj) {
          ...
          toast.error(response.error || "Erro ao buscar CNPJ.");
        }
      },
      onError: (error) => {
        toast.error(
                                ...
                                <TooltipTrigger asChild>
                                  <Button
                                    ...
                                    disabled={
                                      isLoadingCnpjSearch ||
                                      !form.getValues("cpf") ||
                                      form.getValues("cpf").replace(/\D/g, "").length !== 14
                                    }
                                  >
                                    {isLoadingCnpjSearch ? (
